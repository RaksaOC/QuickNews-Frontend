...
export function VideoText({ creator, title, description, onShowArticle }: VideoTextProps) {
    ...
    return (
        <div className={`absolute bottom-4 left-0  w-full px-4 pb-2 flex flex-col gap-2 ${!isShortened ? "bg-gradient-to-b from-transparent via-black/50 to-black" : ""}`}>
            ...
            <div className={` text-white text-sm lg:text-md mt-2  font-bold ${isShortened ? "line-clamp-2" : "line-clamp-none"}`} onClick={() => { setIsShortened(!isShortened) }}>
            ...
            <div className=" text-white text-sm lg:text-md flex font-bold flex-row gap-5 items-center justify-between ">
                <div className="flex flex-row gap-2 items-center ">
                    ...
                    <div className="flex flex-row gap-1 items-center text-xs lg:text-sm cursor-pointer" onClick={() => { router.push(`/${creator.handle}`) }}>
                        <p>{creator.name}</p>
                    ...
                    <button className={`text-white text-sm border bg-gray-500/30 backdrop-blur-sm border-gray-500/50 rounded-full px-2 py-1 ${isFollowing ? "bg-sky-500" : "bg-gray-500/30"} transition-all duration-300`} onClick={() => { setIsFollowing(!isFollowing) }}>
                        {isFollowing ? <div className="flex flex-row gap-1 items-center">
                            <p className="text-xs lg:text-sm">Followed</p>
                            <CheckCircle className="lg:size-4 size-3 text-white fill-sky-500" />
                        </div> : <p className="text-xs lg:text-sm">Follow</p>}
                    </button>
                </div>
                <button className="text-white text-xs lg:text-sm  bg-sky-500  rounded-full px-2 py-1" onClick={onShowArticle}>
                    Article
                </button>
            </div>
        </div >
