    ...
    <div className="bg-[#18181b] rounded-2xl p-3 flex items-center gap-3 mb-4">
      ...
      <div className="flex-1">
        ...
        <div className="flex items-center gap-4">
          ...
          <button className="ml-auto bg-sky-500 text-white text-xs px-4 py-1 rounded-full font-medium">
            {action}
          </button>
        </div>
      </div>
...
export default function CreatorPage() {
  ...
  const [activeTab, setActiveTab] = useState<"videos" | "articles" | "saved">(
    "videos"
  );
  ...
  useEffect(() => {
    ...
    if (userData) {
      setProfile(userData);
      setUserVideos(
        LEGIT_VIDEOS.filter((video) => video.creator.handle === userData.handle)
      );
    }
  }, [handle]);
    ...
    profile && (
      <div className="relative max-h-screen h-screen">
        <div
          ...
          className="bg-black max-h-screen h-full overflow-y-scroll"
        >
          {/* Sticky Header that overlays everything */}
          <div
            className={`sticky top-0 left-0 right-0 flex justify-between items-center px-4 py-4 z-50 transition-colors duration-300 ${
              isScrolledPastCover ? "bg-black" : "bg-transparent"
            }`}
          >
            <button
            ...
            <div className="relative h-60 w-full -mt-20">
              ...
              <div className="absolute bottom-[-32px] flex items-center w-full px-4">
                ...
                <div className="ml-4 flex items-center justify-between w-full ">
                  ...
                  <button
                    className={`text-white text-xs border backdrop-blur-sm rounded-full px-4 py-2 font-medium flex items-center gap-1 transition-all duration-300 ${
                      isFollowing
                        ? "bg-sky-500 border-sky-500"
                        : "bg-gray-500/50 border-slate-500"
                    }`}
                  ...
                  >
                    {isFollowing ? (
                      <div className="flex flex-row gap-1 items-center">
                        ...
                        <CheckCircle
                          size={16}
                          className="text-white fill-sky-500"
                        />
                      </div>
                    ) : (
                      "Follow"
